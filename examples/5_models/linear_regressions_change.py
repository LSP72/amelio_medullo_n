...
def fit_continuous(x, y):
    ...
    return {
        "n": int(len(x1)),
        "equation": f"change = {intercept:.3f} + {slope:.4f} * feature",
        "r2": res.rsquared,
...
def plot_grid(df, y, features):
    ...
    for ax, f in zip(axes, features):
        ...
        if f in CATEGORICAL:
            ...
            if xv.nunique() >= 2:
                ...
                ax.axhline(y=44, color="gray", lw=1, ls="--", alpha=0.7, label="Stroke")
                ax.axhline(y=45.8, color="gray", lw=1, ls="--", alpha=0.7, label="SCI")
                ax.axhline(y=45, color="gray", lw=1, ls="--", alpha=0.7, label="Other")
    ...
    fig.legend()
    fig.show()
    fig.savefig("univariate_regression_grid_change.png", dpi=150, bbox_inches="tight")
    print("\nSaved figure -> univariate_regression_grid_change.png")


def main():
    df = load()
    y = df["__change__"]
    print(f"Outcome: 6MWT change (post - pre). n with outcome = {y.notna().sum()}")
    print(f"Change: mean {y.mean():.1f} m, sd {y.std():.1f}, range [{y.min():.0f}, {y.max():.0f}]\n")
    ...
    for f in FEATURES:
        ...
        r["feature"] = f
        r["kind"] = kind
        r["flag"] = "COUPLED-to-outcome (distrust)" if f in COUPLED else ""
        rows.append(r)
