...
def main():
    df = load()
    y = df["6MWT_m_post"] - df["6MWT_m_pre"]
    print(f"Outcome: 6MWT change. n with outcome = {y.notna().sum()}")
    print(f"6MWT change: mean {y.mean():.1f} m, sd {y.std():.1f}, range [{y.min():.0f}, {y.max():.0f}]\n")
