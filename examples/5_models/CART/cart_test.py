...
def node_table(clf, feature_names, class_names):
    ...
    for i in range(t.node_count):
        feuille = t.children_left[i] == -1  # Check if the left child of the root node is a leaf node (i.e., it has no children). If it is a leaf node, this will return True; otherwise, it will return False.
        ...
        ligne = {
            ...
            "sample_pct": round(100 * n_i / t.n_node_samples[0], 1), # percentage of samples at node i relative to the total number of samples in the dataset (root node = node 0).
            "gini": round(t.impurity[i], 4),
            "rule": ("-" if feuille else
                      f"{feature_names[t.feature[i]]} <= {t.threshold[i]:.2f}"), # only created for internal nodes
...
def improvements(clf, feature_names):
    ...
    for i in range(t.node_count):
        ...
        if l == -1:
            continue #
        w = t.weighted_n_node_samples # take into account any weight attributed to the samples, if any (in case of class imbalance, ponderation, etc.)
        delta_G = t.impurity[i] - (w[l] / w[i] * t.impurity[l] 
                                 + w[r] / w[i] * t.impurity[r])  # Calculation of deltaGini
        out.append({
            "node": i,
            ...
            "delta_balanced": round(delta_G * w[i] / w[0], 4), # avg impurity of children => the delta is what have been gained
        })
...
def pruning_1se(X, y, base_params, cv=10, random_state=42):
    ...
    _type_
        _description_
    """
    track = DecisionTreeClassifier(**base_params,
    ...
    means, std_err = [], []
    for a in alphas:
        m = DecisionTreeClassifier(**base_params, ccp_alpha=a,
                                   random_state=random_state)
    ...
    return {
        ...
        "table": pd.DataFrame({"ccp_alpha": alphas,
                                 "acc_cv": means.round(4),
                                 "se": std_err.round(4)}),
    }
...
def display_metrics(clf, clf_no_pruning, FEATURES, CLASSES, nodes, ameliorations, res_pruning, matrix, perf, acc_apparente, acc_cv):
    ...
    print(export_text(clf, feature_names=FEATURES,
                      class_names=CLASSES, decimals=1))

    print("=" * 72)
    print("NODE TABLE")
    print("=" * 72)
    print(nodes[["node", "type", "n", "sample_pct", "gini",
                  "rule", "prediction"]].to_string(index=False))
    print()
    ...
    print("=" * 72)
    print(f"optimal alpha      : {res_pruning['alpha_best']:.5f} "
          f"(acc CV = {res_pruning['acc_cv_best']:.3f})")
    print(f"retained alpha (1-SE): {res_pruning['alpha_1se']:.5f} "
          f"(acc CV = {res_pruning['acc_cv_1se']:.3f})")
    print(f"leaves without / with pruning : "
          f"{clf_no_pruning.get_n_leaves()} / {clf.get_n_leaves()}")
    ...
    for nom, imp in sorted(zip(FEATURES, clf.feature_importances_),
                           key=lambda x: -x[1]):
    ...
    plot_tree(clf, feature_names=FEATURES, class_names=CLASSES,
...
def main(data_path, features, cat_cols, random_state=42):
    ...
    clf_no_pruning = DecisionTreeClassifier(**params,
            ccp_alpha=0, 
            random_state=random_state
            )
    ...
    clf = DecisionTreeClassifier(
        ...
        ## A JUSTIFIER
        ccp_alpha=res_pruning["alpha_1se"],  # Complexity parameter used for Minimal Cost-Complexity Pruning. A value of 0.0 means no pruning will be performed; here, has been optimised.
        random_state=random_state
        )
    clf.fit(X, y)
    ...
    acc_cv = cross_val_score(
        DecisionTreeClassifier(**params, ccp_alpha=res_pruning["alpha_1se"], random_state=random_state), X, y,
        cv=StratifiedKFold(10, shuffle=True, random_state=random_state)).mean()

    display_metrics(clf, clf_no_pruning, features_names, class_names, nodes, ameliorations, res_pruning, matrix, perf, acc_apparente, acc_cv)
...
if __name__ == "__main__":
    ...
    features = [
            ...
            "Neurol_cond", "Sex", "Nb sessions", "BMI", "Age"
        ]
