...
print(
    f"n = {len(X)} | événements classe rare = {n_events} | "
...
preprocess = ColumnTransformer(
    [
        (
            "num",
            Pipeline([("imp", SimpleImputer(strategy="median")), ("sc", StandardScaler())]),  # robuste sur petit n
            numeric_cols,
        ),
        (
            "cat",
...
clf = LogisticRegressionCV(
    Cs=10,
    cv=inner_cv,  # choisit C par CV interne, tout seul
    penalty="l2",
    solver="liblinear",
...
print(
    (
        pd.DataFrame({"feature": feat, "coef": coefs, "odds_ratio": np.exp(coefs)})
...
perf = pd.DataFrame(
    {
        m: {
            ...
            "min": res[f"test_{m}"].min(),
            "max": res[f"test_{m}"].max(),
        }
        for m in scoring
    }
).T
...
meta = pd.DataFrame(
    {
        "champ": [
            "date",
            "n_patients",
            "n_evenements_classe_rare",
            "taux_classe_majoritaire",
            "C_retenu",
            "penalty",
            "class_weight",
            "plis_externes",
        ...
        "valeur": [
            ...
            round(float(pipe.named_steps["clf"].C_[0]), 4),
            "l2",
            "balanced",
            outer_k,
            inner_cv,
