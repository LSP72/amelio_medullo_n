...
def screen_nominal(x, y):
    ...
    return dict(
        n=len(yy), type="nominal", AUC_apparent=auc, p_value=p_lr, MW_p=np.nan, OR_per_unit=np.nan, OR_per_SD=np.nan
    )


def main(data_path, cols_to_keep, dataset, nominal_cols=NOMINAL_COLS, out_dir="results/uni_multi_variate"):
    os.makedirs(out_dir, exist_ok=True)

    X, y = load_data(data_path, cols_to_keep)
    print(
        f"n total chargé = {len(y)} | " f"répondeurs = {int((y == 1).sum())} | non-répondeurs = {int((y == 0).sum())}"
    )

    results = []
    for col in cols_to_keep:
        try:
            r = screen_nominal(X[col], y) if col in nominal_cols else screen_continuous(X[col], y)
    ...
    df = pd.DataFrame(results).set_index("Biomarker").sort_values("AUC_apparent", ascending=False)
