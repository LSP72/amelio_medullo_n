...
def collect_data(data, first_sheet, intervention):
    ...
    if check_patients == True:
        list_patients_intervention = ProcessPatients.collect_patients_for_chosen_intervention(data=data, main_info=first_sheet, intervention=intervention)
        ...
        raise ValueError("/!\ Some patients in the sub-sheets are not in the main sheet.\nThus no demographic data are available.")
...
if __name__ == "__main__":
    ...
    desired_lists_cols ={
        "bilan_init_birdlm": ['IEP', 'IPP', 'Date_Formulaire', 'Poids_dosage',
            'Taille_dosage', 'Contre_Indications', 'Cerebrolese',
            'BlesseMedullaire',],
        "bilan_init_6m": ['IEP', 'IPP','Age', 'Distance_parcourue',
