...
def lasso_stability_over_splits(X, y, n_iterations=100, cv=5, test_size=0.2):
    ...
    for rs in range(1, n_iterations + 1):
        selected, coefs, acc = lasso_pipeline_one_split(
            X, y, random_state=rs, cv=cv, test_size=test_size
        )
    ...
    stability_df = pd.DataFrame({
        ...
        'selection_freq_%': freq_df['frequency_%']
...
def main(data_path, cols_to_keep, classif=True, n_iterations=100):
    ...
    stability_df, accuracies = lasso_stability_over_splits(
        X, y, n_iterations=n_iterations
    )
